...
Equipment :: Equipment(int hpp , int atkp , int defp){
	hpmax = hpp ;
	atk = atkp ;
	def = defp ;
}
...
vector<int> Equipment::getStat(){
	vector <int> stateq = {hpmax,atk,def} ;
	return stateq ;
}

void Unit::equip(Equipment *hpp){
	hpmax = hpeqp ;
	atk = a ;
	def = d ;
	vector<int> n = hpp -> getStat() ;
	hpmax += n[0] ;
	atk += n[1] ;
	def += n[2] ;
	hp = min(hp, hpmax) ;
